...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/dashboard", isAuthenticated, async (req, res) => {
    try {
      ...
      const birthdayPerson = await db.query.user.findFirst({
        where: (user) => {
          const birthDate = new Date(user.birthdate);
          return and(
            eq(birthDate.getMonth(), todayDate.getMonth()),
            eq(birthDate.getDate(), todayDate.getDate())
          );
        }
      });
      ...
      res.json({
        stats: {
          ...
          birthdayPerson: birthdayPerson ? { name: birthdayPerson.name, id: birthdayPerson.id } : null
        },
