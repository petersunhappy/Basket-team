...
import { Check, Clock, Bell, CheckCircle, MessageSquare, Calendar } from "lucide-react";
...
export default function Dashboard() {
  ...
  const {
    ...
    todayTraining = { id: null, exercises: [], focus: "" },
    activities = [],
    events = [],
    news = []
  } = dashboardData || {};
        ...
        <main className="flex-1 p-4 md:p-6">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              />
              {stats.birthdayPerson && (
                <StatsCard
                  title="Aniversariante do Dia"
                  value={`Parabéns ${stats.birthdayPerson.name}!`}
                  icon={Cake}
                  color="accent"
                  subtext="Feliz Aniversário!"
                  isSpecial={true}
                />
              )}
            </div>
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <Card className="overflow-hidden">
                  <div className="bg-secondary p-4 text-white flex justify-between items-center">
                    ...
                    {todayTraining ? (
                      <span className="text-sm bg-accent text-secondary px-3 py-1 rounded-full font-semibol">
                        Treino #{todayTraining.id}
                      </span>
                    ) : (
                      // Opcional: Mostrar algo caso todayTraining seja nulo
                      <span className="text-sm text-gray-500">Nenhum treino hoje</span>
                    )}
                  </div>
                  ...
                  <CardContent className="p-6">
                    <div className="mb-6">
                      <h3 className="font-heading font-semibold text-lg text-secondary mb-2">
                        Foco: {todayTraining?.focus || "Nenhum treino hoje"}
                        
                      </h3>
                      <p className="text-neutral-medium">
                        {todayTraining?.exercises.length > 0 
                          ? "Complete todos os exercícios abaixo e registre seus resultados."
