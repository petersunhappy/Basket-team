...
interface StatsCardProps {
  ...
  subtext?: string;
  date?: string;
  isSpecial?: boolean; // Added isSpecial prop
}
...
export function StatsCard({
  ...
  subtext,
  date,
  isSpecial = false, // Added default value for isSpecial
}: StatsCardProps) {
    ...
    <div className={`bg-white rounded-lg shadow-sm p-6 border-l-4 ${borderColor}`}>
      ...
      {subtext && (
        <p className="mt-4 text-sm">
          ...
          {isSpecial && <span className="text-green-500"> Parabéns!</span>} {/* Added birthday message */}
        </p>
      )}
    </div>
  );
